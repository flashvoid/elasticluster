...
CLOUD_PROVIDER_SCHEMAS = {
    ...
    'openstack': {
        ...
        Optional("manage_anti_affinity_groups"): boolean,
        Optional("anti_affinity_group_prefix"): nonempty_str,
    },

    'openstack-muz': {
        "provider": 'openstack-muz',
        "backends": existing_file,
        'zones': {
            str: {
                str: str,
            },
...
CLOUD_PROVIDERS = {
    ...
    'openstack-muz': ('elasticluster.providers.openstack-muz',      'OpenStackCloudProvider'),
    ...
    * 2nd-level keys are then the names given to such sections. For example,
    ...
    for sect_name, sect_items in raw_config.iteritems():
        ...
        if path[0] == 'cluster' and len(path) > 2:
            path.insert(2, 'nodes')
        if path[0] == 'cloud' and len(path) > 2:
            path.insert(2, 'zones')
...
class Creator(object):
    ...
    def create_cloud_provider(self, cluster_template):
        ...
        try:
            ...
            if cloud_conf['provider'] == 'openstack-muz':
                zones = self.cluster_conf['slurm']['cloud']['zones']
                for zonename, zoneconfig in zones.iteritems():
                    zone_provider_config = zoneconfig.copy()
                    provider = zone_provider_config.pop('provider')
                    zone_provider = _get_provider(provider, CLOUD_PROVIDERS)
                    zoneconfig['api'] = zone_provider(**zone_provider_config)
